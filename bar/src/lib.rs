...
pub async fn send_to_s3(file_name: String, file: &[u8]) -> Result<()> {
    ...
    let (x_amz_date, ddate) = get_x_amz_date();
    ...
    */
    ...
    let x_amz_content_sha256 = sha256hash_hex_encoded(file).unwrap();
    ...
    let sha256_of_canonical_request = sha256hash_hex_encoded(canonical_request.as_bytes()).unwrap();
    println!(
    ...
    */
    ...
    .unwrap();
    let date_region_key = hmac_sha256(date_key, region.clone().into_bytes()).unwrap();
    let date_region_service_key =
        hmac_sha256(date_region_key, service.clone().into_bytes()).unwrap();
    ...
    let signature = hmac_sha256(signing_key, string_to_sign.clone().into_bytes()).unwrap();

    let signature_string = hex_encoded(signature).unwrap();
...
pub fn get_x_amz_date() -> (String, String) {
...
pub fn hmac_sha256(key: Vec<u8>, text: Vec<u8>) -> Result<Vec<u8>> {
...
pub fn sha256hash_hex_encoded(data: &[u8]) -> Result<String> {
...
pub fn hex_encoded(a: Vec<u8>) -> Result<String> {
    Ok(hex::encode(a))
}
